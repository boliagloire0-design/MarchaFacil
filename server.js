    ...
    <body style="display:flex; align-items:center;"><div class="container">
        ...
        <div class="card">
            ...
            <p style="text-align:center; font-size:12px; margin-top:15px;">
                <a href="/signup" style="color:var(--primary)">${t.signup}</a>
            </p>
        </div>
    </div></body></html>`);
});
    ...
    <body style="display:flex; align-items:center;"><div class="container">
        ...
        <div class="card">
            ...
            <form action="/signup" method="POST">
                <input type="text" name="name" placeholder="Full Name / Nome Completo" required>
                <input type="email" name="email" placeholder="Email" required>
                <input type="password" name="passcode" placeholder="PIN (4-6 digits)" required>
                <input type="hidden" name="referredBy" value="${refId}">
                <button type="submit">${t.signup}</button>
            </form>
            <p style="text-align:center; font-size:12px;"><a href="/" style="color:var(--primary)">Already have an account? Login</a></p>
        </div>
    </div></body></html>`);
});
...
app.post('/signup', async (req, res) => {
    try {
        const { name, email, passcode, referredBy } = req.body;
        const newUser = new User({
            name,
            email: email.toLowerCase(),
            passcode,
            referredBy: mongoose.Types.ObjectId.isValid(referredBy) ? referredBy : null
        });
        await newUser.save();
        res.send("<script>alert('Account Created! Please Login.'); window.location='/';</script>");
    } catch (err) {
        res.send("Error creating account. Email might already exist.");
    }
});
...
app.get('/agent', async (req, res) => {
    ...
    <div class="container">
        ...
        <script>function copyLink(){var t=document.getElementById("refLink");t.select(),t.setSelectionRange(0,99999),navigator.clipboard.writeText(t.value),alert("Link Copied!")}</script>
        ...
        ${pendings.map(p => `
            <div class="card">
                ...
                <form action="/confirm-deposit" method="POST"><input type="hidden" name="uid" value="${p._id}"><button type="submit">${t.confirm}</button></form>
                <form action="/reject-deposit" method="POST" style="margin-top:5px;"><input type="hidden" name="uid" value="${p._id}"><input type="text" name="reason" placeholder="${t.reason}" required><button type="submit" style="background:var(--admin); color:white">${t.reject}</button></form>
            </div>`).join('')}
        <div class="card">
...
// =============================================================
// ✅ UTILITY & HELPERS (PDF, Lang, Deposits)
// =============================================================
app.get('/receipt/:txIndex', async (req, res) => {
    ...
    doc.moveDown().fontSize(12).text(`User: ${u.email}`).text(`Operation: ${tx.type}`).text(`Amount: ${tx.amount} ${CURRENCY}`).text(`Fee: ${tx.fee || 0} ${CURRENCY}`).text(`Date: ${tx.date}`);
    doc.moveDown().fontSize(8).text('MarchaFácil © 2026 - Maputo, MZ', { align: 'center' });
    doc.end();
});

app.post('/deposit', async (req, res) => {
    await User.findByIdAndUpdate(req.session.userId, { pendingDeposit: { amount: parseFloat(req.body.amount), status: "Pending", date: new Date() } });
